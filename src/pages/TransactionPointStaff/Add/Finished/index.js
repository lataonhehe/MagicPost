...
import { customerCells } from "~/components/UI/TableCell";
...
function FinishedTransaction() {
  ...
  const tableName = "Tạo đơn hàng người nhận";
  ...
  const fetchData = async () => {
    try {
      ...
      const apiUrl = "http://127.0.0.1:8000/Transaction/transaction_employee/get_customer_shipment_list";
  ...
  const handleAccept = async () => {
    try {
      const token = localStorage.getItem("Token");
      const response = await fetch("http://127.0.0.1:8000/Transaction/transaction_employee/create_transaction_to_receiver", {
        method: "POST",
        headers: {
          Authorization: `Token ${token}`,
          "Content-Type": "application/json",
