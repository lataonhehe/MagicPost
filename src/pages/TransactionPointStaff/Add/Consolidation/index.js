...
function TransStaffAddConsol() {
  ...
  const fetchData = async () => {
    try {
      ...
      const apiUrl = "http://127.0.0.1:8000/Transaction/employee/get_shipment_list";
  ...
  const handleAccept = async () => {
    try {
      const token = localStorage.getItem("Token");
      const response = await fetch("http://127.0.0.1:8000/Transaction/transaction_employee/shipment_to_consolidation", {
        method: "POST",
        headers: {
          Authorization: `Token ${token}`,
          "Content-Type": "application/json",
