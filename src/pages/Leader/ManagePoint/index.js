...
import { departmentCells } from "~/components/UI/TableCell";
...
function ManagePoint() {
  ...
  const cells = departmentCells;
  const hasCheckbox = false;
  const tableName = "Thống kê điểm";
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch("http://127.0.0.1:8000/Transaction/list_department", {
        method: "GET",
        headers: {
      ...
      updateRows(data.department_list);
    } catch (error) {
